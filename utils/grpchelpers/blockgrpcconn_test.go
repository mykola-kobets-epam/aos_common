...
func getNodeInfo(connection *grpchelpers.BlockingGRPCConn) error {
...
func TestStoppedConnectionFails(t *testing.T) {
	...
	blockingConn := grpchelpers.NewBlockingGRPCConn()
	blockingConn.Set(grpcConn)

	if err := getNodeInfo(blockingConn); err != nil {
...
func TestRPCBlockedUntilConnectionReset(t *testing.T) {
	...
	go func() {
		rpcResultChan <- getNodeInfo(blockingConn)
	}()

	time.Sleep(testTimeout / 2) // give time for goroutine to start

	// Start the server, restore connection & wait until previous rpc succeed
	err = testServer.RestartServer([]grpc.ServerOption{grpc.ConnectionTimeout(testTimeout)})
	if err != nil {
		t.Fatalf("Connection failed, err=%v", err)
	}
